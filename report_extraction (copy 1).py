    ...
    def select_and_login_and_load_steps(self):
        ...
        for account in credentials:
            ...
            if new_status == "valid":
                ...
                print(f"LOGIN_ERROR:{user_folder}")
    ...
    def perform_check_password_expired(self, step_name, images):
        ...
        try:
            ...
            if len(images) > 0 and images[0]:
                if StepExecutor.check_image_exists(images[0], report_name=self.report_name):
                    log_message(f"Expired-password detected for user {username}", WARNING)
                    print(f"PASSWORD_EXPIRED:{username}")
                    self._update_credentials_status(username, status="expired")
                    raise Exception("Password expired detected")
            # Login error
            if len(images) > 1 and images[1]:
                if StepExecutor.check_image_exists(images[1], report_name=self.report_name):
