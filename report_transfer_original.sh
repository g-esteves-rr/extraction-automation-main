...
for file in "$LOCAL_DIR"*; do
  ...
  for target in "${targets[@]}"; do
    ...
    echo "Uploading $file_base to $url"

    curl -T "$file" --ftp-method nocwd --ftp-pasv --ftp-ssl \
         -u "$FTP_USER:$FTP_PASS" -Q "TYPE I" "$url"

    if [ $? -eq 0 ]; then
      echo "$(date) SUCCESS: $file_base -> $server:$port$dest_path"
      transfer_success=true
    else
      echo "$(date) FAILED: $file_base -> $server:$port$dest_path"
    fi
  done
